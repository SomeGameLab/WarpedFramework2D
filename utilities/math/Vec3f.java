...
public class Vec3f { //NOTE -> when used in combination with MatrixD will be considered a ROW VECTOR
	...
	public float difference(Vec3f vector) { //FIXME is this correct?
	...
	public static Vec3f generateRandomVector(float xBound, float yBound, float zBound) {
		Vec3f result = new Vec3f();
		result.x = Maths.random.nextFloat(xBound);
		result.y = Maths.random.nextFloat(yBound);
		result.z = Maths.random.nextFloat(zBound);
		return result;		
	}
	public static Vec3f generateRandomVector(float xMin,float xMax, float yMin, float yMax, float zMin, float zMax) {
		Vec3f result = new Vec3f();
		result.x = Maths.random.nextFloat(xMin,xMax);
		result.y = Maths.random.nextFloat(yMin,yMax);
		result.z = Maths.random.nextFloat(zMin,zMax);
		return result;		
	}
