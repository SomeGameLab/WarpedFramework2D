...
public class Vec3i { // NOTE -> when used in combination with MatrixD will be considered a ROW VECTOR
	...
	public int getValue(int index) {
		...
		case 1: return y;
		case 2: return z;
		default: System.err.println("Vec3i -> getValue() -> invalid index : " + index);	return -1;
		}	
	}
	...
	public void subtract(Vec3i vec) {x -= vec.x; y -= vec.y; z -= vec.z;}
	...
	public static Vec3i generateRandomVector(Vec2i xBounds, Vec2i yBounds, Vec2i zBounds) {
		Vec3i result = new Vec3i();
		result.x = Maths.random.nextInt(xBounds.x, xBounds.y);
		result.y = Maths.random.nextInt(yBounds.x, yBounds.y);
		result.z = Maths.random.nextInt(zBounds.x, zBounds.y);
		return result;		
	}
	public static Vec3i generateRandomVector(Vec2i bounds) {
		Vec3i result = new Vec3i();
		result.x = Maths.random.nextInt(bounds.x, bounds.y);
		result.y = Maths.random.nextInt(bounds.x, bounds.y);
		result.z = Maths.random.nextInt(bounds.x, bounds.y);
		return result;		
	}
	public static Vec3i generateRandomVector(int xBound, int yBound, int zBound) {
		...
		result.z = Maths.random.nextInt(zBound);
		return result;		
	}
